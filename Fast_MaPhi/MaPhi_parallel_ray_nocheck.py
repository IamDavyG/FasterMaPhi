    ...
    if not os.path.exists(mol_path):
        ...
        with open(save_path,'a+') as data_file:
            ...
            for k in molecules_F:
                ...
                @ray.remote
                def normterm(molecule_file):
                    normterm = get_val("orca_input.out","****ORCA TERMINATED NORMALLY****",0,0,-1,1)
                    return normterm
                
                @ray.remote
                def convterm(molecule_file):
                    convterm = get_val("orca_input.out","The optimization did not converge",0,0,-1,1)
                    return convterm
                
                rNormterm = normterm.remote(molecule_file)
                rConvterm= convterm.remote(molecule_file)
                
                normal_term, conve_term = ray.get([rNormterm, rConvterm])
                ...
                if normal_term == check_norm and conve_term == None:
                    ...
                    rCheck1 = check_1.remote(molecule_file)    
                    rCheck2 = check_2.remote(molecule_file)
                    
                    electrons, converg = ray.get([rCheck1, rCheck2])
                    ...
                    homo, homo1, homo2, lumo, lumo1, lumo2 = calc_homo_lumo_values(molecule_file, electrons, converg)
                    
                    @ray.remote
                    def HOMO(molecule_file, homo=homo, converg=converg):
                        HOMO = float(get_val("orca_input.out","OCC",homo,16,29,converg))
                        return HOMO
                    
                    @ray.remote
                    def HOMO1(molecule_file, homo1=homo1, converg=converg):
                        HOMO1 = float(get_val("orca_input.out","OCC",homo1,16,29,converg))
                        return HOMO1

                    @ray.remote
                    def HOMO2(molecule_file, homo2=homo2, converg=converg):
                        HOMO2 = float(get_val("orca_input.out","OCC",homo2,16,29,converg))
                        return HOMO2                   
                    
                    @ray.remote
                    def LUMO(molecule_file, lumo=lumo, converg=converg):
                        LUMO = float(get_val("orca_input.out","OCC",lumo,16,29,converg))
                        return LUMO
                    
                    @ray.remote
                    def LUMO1(molecule_file, lumo1=lumo1, converg=converg):
                        LUMO1 = float(get_val("orca_input.out","OCC",lumo1,16,29,converg))
                        return LUMO1
                    
                    @ray.remote
                    def LUMO2(molecule_file,lumo2=lumo2, converg=converg):
                        LUMO2 = float(get_val("orca_input.out","OCC",lumo2,16,29,converg))
                        return LUMO2
                    
                    rHOMO = HOMO.remote(molecule_file, homo)
                    rHOMO1 = HOMO1.remote(molecule_file, homo1)
                    rHOMO2 = HOMO2.remote(molecule_file, homo2)
                    rLUMO = LUMO.remote(molecule_file, lumo)
                    rLUMO1 = LUMO1.remote(molecule_file, lumo1)
                    rLUMO2 = LUMO2.remote(molecule_file, lumo2)
                    
                    HOMO, HOMO1, HOMO2, LUMO, LUMO1, LUMO2 = ray.get([rHOMO, rHOMO1, rHOMO2, rLUMO, rLUMO1, rLUMO2])
                    
                    print(HOMO, HOMO1, HOMO2, LUMO, LUMO1, LUMO2)
                    
                    hardness = LUMO - HOMO
                    
                    @ray.remote
                    def electronegativity(molecule_file, HOMO, LUMO):
                        electronegativity = float((LUMO + HOMO)/2)
                        return electronegativity
                    
                    @ray.remote
                    def electrophilicity(molecule_file, HOMO, LUMO):
                        electrophilicity = float(((LUMO + HOMO)**2)/(4*(LUMO-HOMO)))
                        return electrophilicity
                    
                    @ray.remote
                    def ElectronDonatingPower(molecule_file, HOMO, LUMO, hardness):
                        EDP = (3*float(HOMO)+float(LUMO))**2/(16*float(hardness))
                        return EDP
                    
                    @ray.remote
                    def ElectronAcceptingPower(molecule_file, HOMO, LUMO, hardness):
                        EAP = (float(HOMO)+(3*float(LUMO)))**2/(16*hardness)
                        return EAP            

                    @ray.remote
                    def dipole(molecule_file):
                        dipole = float(get_val("orca_input.out","DIPOLE MOMENT",9,-11,-1,1))
                        return dipole
                    
                    @ray.remote
                    def quadXX(molecule_file):
                        quadXX = float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,9,22,1))
                        return quadXX

                    @ray.remote
                    def quadYY(molecule_file):
                        quadYY = float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,23,35,1))
                        return quadYY                    
                    
                    @ray.remote
                    def quadZZ(molecule_file):
                        quadZZ= float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,36,48,1))
                        return quadZZ
                    
                    @ray.remote
                    def quadXY(molecule_file):
                        quadXY = float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,49,60,1))
                        return quadXY
                    
                    @ray.remote                    
                    def quadXZ(molecule_file):
                        quadXZ = float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,61,74,1))
                        return quadXZ
                    
                    @ray.remote
                    def quadYZ(molecule_file):
                        quadYZ = float(get_val("orca_input.out","QUADRUPOLE MOMENT (A.U.)",6,75,87,1))
                        return quadYZ
                    
                    @ray.remote
                    def IsotropicQuad(molecule_file):
                        IsoQuad = float(get_val("orca_input.out","Isotropic quadrupole",0,-11,-1,1))
                        return IsoQuad
                    
                    @ray.remote 
                    def FinalSPEnergy(molecule_file):
                        with open('orca_input.out') as fp:
                            for cnt, line in enumerate(fp):
                    ...
                    rElectronegativity = electronegativity.remote(molecule_file, HOMO, LUMO)
                    rElectrophilicity = electrophilicity.remote(molecule_file, HOMO, LUMO)
                    rEDP = ElectronDonatingPower.remote(molecule_file, HOMO, LUMO, hardness)
                    rEAP = ElectronAcceptingPower.remote(molecule_file, HOMO, LUMO, hardness)

                    rDipole = dipole.remote(molecule_file)
                    rQuadXX = quadXX.remote(molecule_file)
                    rQuadYY = quadYY.remote(molecule_file)
                    rQuadZZ = quadZZ.remote(molecule_file)
                    rQuadXY = quadXY.remote(molecule_file)
                    rQuadXZ = quadXZ.remote(molecule_file)
                    rQuadYZ = quadYZ.remote(molecule_file)
                    rIsoQuad = IsotropicQuad.remote(molecule_file)

                    rFinalSPEnergy = FinalSPEnergy.remote(molecule_file)
                    
                    electronegativity, electrophilicity, ElectronDonatingPower, \
                    ElectronAcceptingPower = ray.get([rElectronegativity, rElectrophilicity, \
                                                      rEDP, rEAP])

                    NetElectrophilicity = (ElectronDonatingPower + ElectronAcceptingPower)

                    dipole, quadXX, quadYY, quadZZ, quadXY, quadXZ, quadYZ, IsotropicQuad = \
                    ray.get([rDipole, rQuadXX, rQuadYY, rQuadZZ, rQuadXY, rQuadXZ, rQuadYZ, rIsoQuad])

                    tot_e = ray.get(rFinalSPEnergy)
